...
class Trainer:
    def __init__(self, config):
        ...
        real_dataset = ODERegressionLMDBDataset(
                config.real_data_path, max_pair=int(1e8))
        sampler = torch.utils.data.distributed.DistributedSampler(
            dataset, shuffle=True, drop_last=True)
        dataloader = torch.utils.data.DataLoader(
            dataset, batch_size=config.batch_size, sampler=sampler)
        real_sampler = torch.utils.data.distributed.DistributedSampler(
            real_dataset, shuffle=True, drop_last=True)
        real_dataloader = torch.utils.data.DataLoader(
            real_dataset, batch_size=config.batch_size, sampler=real_sampler)
        self.dataloader = cycle(dataloader)
        self.real_dataloader = cycle(real_dataloader)
        self.step = 0
        self.max_grad_norm = 10.0
        self.previous_time = None
        self.ema_reg_losses = [None] * len(config.denoising_step_list)
        self.alpha = 0.95
        self.k = 3.0
    ...
    def train_one_step(self):
        ...
        if TRAIN_GENERATOR:
            ...
            video_batch = next(self.real_dataloader)
            video_text_prompts = video_batch["prompts"]
            video_latent = video_batch["ode_latent"][:, -1].to(
                device=self.device, dtype=self.dtype)
            # print(video_latent.shape)
            # video_latent = video_batch["ode_latent"].to(
            #     device=self.device, dtype=self.dtype)
            # video_latent = video_latent.reshape(1, 21, 16, 60, 106)
            video_latent = video_latent[..., :104]

            video_conditional_dict = self.distillation_model.text_encoder(text_prompts=video_text_prompts)

            current_reg_loss, idx = self.distillation_model.reg_loss(
                image_or_video_shape=image_or_video_shape,
                conditional_dict=video_conditional_dict,
                unconditional_dict=unconditional_dict,
                clean_latent=video_latent
            )
            if self.config.use_reg_weight:
                idx = int(idx[0][0])
                if self.ema_reg_losses[idx] is None:
                    self.ema_reg_losses[idx] = current_reg_loss.detach()
                else:
                    self.ema_reg_losses[idx] = self.alpha * self.ema_reg_losses[idx] + (1 - self.alpha) * current_reg_loss.detach()
                

                weight = 1 - torch.sigmoid(self.k * (current_reg_loss.detach() - self.ema_reg_losses[idx]))
                print(f"weight:{weight},current_reg_loss:{current_reg_loss},idx:{idx},ema_reg_losses:{self.ema_reg_losses[idx]}")
                generator_loss += weight * current_reg_loss
            else:
                generator_loss += current_reg_loss
